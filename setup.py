...
try:
    ...
    print >> sys.stderr, 'To build pinyto-desktop-todo you need https://launchpad.net/python-distutils-extra'
    sys.exit(1)
assert DistUtilsExtra.auto.__version__ >= '2.18', 'needs DistUtilsExtra.auto >= 2.18'

def update_config(libdir, values = {}):

    filename = os.path.join(libdir, 'pinyto_desktop_todo_lib/pinyto_desktop_todoconfig.py')
    oldvalues = {}
    try:
        fin = file(filename, 'r')
...
def move_desktop_file(root, target_data, prefix):
    ...
    old_desktop_path = os.path.normpath(root + target_data +
                                        '/share/applications')
    old_desktop_file = old_desktop_path + '/pinyto-desktop-todo.desktop'
    desktop_path = os.path.normpath(root + prefix + '/share/applications')
    desktop_file = desktop_path + '/pinyto-desktop-todo.desktop'
    ...
    if not os.path.exists(old_desktop_file):
        ...
        desktop_file = desktop_path + '/extras-pinyto-desktop-todo.desktop'
        try:
...
def update_desktop_file(filename, target_pkgdata, target_scripts):
    ...
    try:
        ...
        for line in fin:
            if 'Icon=' in line:
                line = "Icon=%s\n" % (target_pkgdata + 'media/pinyto-desktop-todo.svg')
            elif 'Exec=' in line:
                cmd = line.split("=")[1].split(None, 1)
                line = "Exec=%s" % (target_scripts + 'pinyto-desktop-todo')
                if len(cmd) > 1:
...
class InstallAndUpdateDataDirectory(DistUtilsExtra.auto.install_auto):
    def run(self):
        ...
        target_pkgdata = target_data + 'share/pinyto-desktop-todo/'
        target_scripts = '/' + os.path.relpath(self.install_scripts, self.root) + '/'

        values = {'__pinyto_desktop_todo_data_directory__': "'%s'" % (target_pkgdata),
...
DistUtilsExtra.auto.setup(
    name='pinyto-desktop-todo',
    version='0.1',
    license='GPL-3',
    author='Johannes Merkert',
    author_email='jonny@pinyto.de',
    description='This is the Pinyto ToDo-List application for Ubuntu.',
    long_description='You can view and edit your ToDo-List in Pinyto with this Application. Pinyto is a personal cloud database and app infrastructure used to collect, sync, analyze and access data from different devices. This application depends on the Pinyto daemon.',
    #url='https://launchpad.net/pinyto-desktop-todo',
